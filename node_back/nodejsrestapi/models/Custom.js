...
var Custom2 = {
	...
	getInvoices:function(callback){
		return db.query("select p1.value from  (select @rownum:=@rownum+1 rank, p.value from miscellaneous p, (SELECT @rownum:=0) r where param='invoice' order by param desc)  as p1 order by p1.rank desc; ",callback);
	},
